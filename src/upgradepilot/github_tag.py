...
```text
repository + requested tag
        │
        ▼
Git ref: refs/tags/{tag}
        │
        ├── object.type == commit
        │       └── resolved commit
        │
...
A Git *reference* is a name such as ``refs/tags/v2.8.4``. A lightweight tag reference
points directly to a commit. An annotated tag reference points to a Git tag object,
which in turn points to another Git object and therefore must be peeled until a commit
is reached.

This module owns exact tag-reference identity and bounded tag-object peeling. It does
not infer tag names from release titles, acquire changelog files, interpret release
prose, or make compatibility, safety, and maintainer-action decisions.
"""
...
class GitHubTagCommitEvidence:
    """Exact tag-reference identity plus the immutable commit reached by peeling."""
    ...
    def peel_depth(self) -> int:
        """Return how many annotated Git tag objects were followed."""
...
class GitHubTagCommitProblem:
    """Why one exact version tag could not be resolved to an immutable commit."""
...
class GitHubTagCommitClient(GitHubApiClient):
    ...
    def resolve_tag_to_commit(
        ...
        """Resolve one exact tag reference, peeling annotated tag objects if needed.

        The supplied tag name is treated as input identity, not as a search hint. The
        returned Git ref must exactly equal ``refs/tags/{requested_tag}`` before any
        object SHA can be trusted.
        """

        repository = validate_repository(repository)
        requested_tag = _nonempty_trimmed_text(requested_tag, "requested_tag")

        encoded_tag = quote(requested_tag, safe="")
        ref_url = self.api_url(f"/repos/{repository}/git/ref/tags/{encoded_tag}")
    ...
    def _peel_annotated_tag(
        ...
        """Follow at most ``max_peel_depth`` tag objects until one commit is reached."""
        ...
        while True:
            # A cycle means the object graph cannot honestly establish one terminating
            # commit. Detect it before another network request is made.
...
class _UnsupportedGitObjectType(ValueError):
    def __init__(self, object_type: str, object_sha: str) -> None:
        super().__init__(f"Git tag resolution encountered unsupported object type {object_type!r}.")
