...
Step 5 proved that UpgradePilot can acquire a caller-supplied changelog path at the
immutable commit named by a dependency-version tag. Step 7 needs one additional source
boundary before normal runtime can use that capability: find one unambiguous admitted
Markdown changelog path without package-specific constants.
...
→ Git commit object
→ exact root tree SHA
→ recursive Git tree listing
→ admitted changelog basename filter
→ one DiscoveredChangelogPath
   or explicit ChangelogPathDiscoveryProblem
```

This module performs source-location discovery only. It does not read the selected file,
interpret Markdown release sections, extract support-drop meaning, call a model, or make
target-Python/compatibility decisions.
"""
...
# Initial runtime support is intentionally Markdown-only because Step 7B's structural
# release-window contract is Markdown ATX headings. Adding another documentation format
# requires its own deterministic section grammar rather than silently widening this list.
...
type ChangelogPathDiscoveryResult = (
    DiscoveredChangelogPath | ChangelogPathDiscoveryProblem
)
...
class GitHubChangelogPathClient(GitHubApiClient):
    ...
    def discover(
        ...
        """Return exactly one admitted path or an explicit bounded discovery problem."""
...
def _validate_commit_sha(value: str) -> str:
    if not isinstance(value, str) or _COMMIT_SHA.fullmatch(value) is None:
        raise ValueError("commit_sha must be a 40- or 64-character hexadecimal object ID.")
    return value
