...
def validate_support_drop_candidates(
    ...
    grouped: dict[tuple[str, str], list[GroundedUpstreamClaimSource]] = {}
...
def _ground_candidate(
    ...
    if not _quote_contains_python_line(candidate.source_quote, candidate.python_line):
        return _problem(
            "source_quote_not_grounded",
            interval,
            (
...
def _validate_candidate_structure(candidate: object) -> str | None:
    ...
    if type(candidate.quote_start) is not int or type(candidate.quote_end) is not int:
...
def _quote_contains_python_line(quote: str, python_line: str) -> bool:
    token = re.compile(rf"(?<![0-9.]){re.escape(python_line)}(?![0-9]|\.[0-9])")
...
__all__ = (
    ...
    "CandidateUpstreamClaimState",
    "GroundableUpstreamClaimSourceKind",
    "GroundedPythonSupportDropClaim",
    "GroundedUpstreamClaimSource",
    "UpstreamSupportDropClaimProblem",
    "UpstreamSupportDropClaimProblemState",
    "UpstreamSupportDropClaimResult",
