...
**Status:** Completed — B2 entry accepted on 2026-07-24  
**Activated:** 2026-07-23  
**Completed:** 2026-07-24  
**Owner:** Ali Rajabi  
**Parent route:** [`UPGRADEPILOT_90_DAY_PLAN.md`](UPGRADEPILOT_90_DAY_PLAN.md)  
**B2 plan:** [`B2_PUBLIC_PR_VERTICAL_SLICE_PLAN.md`](B2_PUBLIC_PR_VERTICAL_SLICE_PLAN.md)  
...
B1 is complete. This file preserves the requirements used to freeze the minimum credible real public PR vertical slice before B2 implementation began. Current execution is controlled by the parent route, the B2 plan, the operating guide, and implemented truth.
...
- The active package was reset to a dependency-free marker with no claimed runtime behavior.
...
This meant B1 defined the new responsibility from evidence and specifications without source inheritance, and the first executable slice had to resemble the eventual product from its initial input through its visible output.
...
The responsibility must begin with a public GitHub repository and Dependabot PR locator and cover, at minimum:
...
- the minimum relevant exact-head CI/check evidence available through the selected authorized interface;
...
- capture of acquired responses or normalized evidence for deterministic testing and later replay;
- no target mutation and no private-repository access.

The responsibility must be complete enough to behave like the eventual product, but small enough to implement and understand without persistence, services, queues, models, agents, deployment infrastructure, exhaustive repository analysis, or the full future artifact family.

### 2. Define the live input and acquisition boundary

The initial user-facing input should be no more than the public repository identity and Dependabot PR number or an equivalent public PR URL.
...
- what happens when information is missing, inaccessible, stale, malformed, contradictory, or changes during acquisition;
...
No caller may provide an unexplained final decision, hidden expected action, repository-specific decision rule, or unlabeled semantic conclusion as product input.
...
Captured responses may reproduce source evidence. They must not supply the product decision to runtime logic. Expected decisions may exist only in tests.
...
3. complexity, learning cost, maintenance, security, upgrade burden, and reversal are explicit;
4. Ali understands and accepts the tradeoff;
5. the B2 plan and tests prove the admitted use.

This comparison applies especially to HTTP access, data validation, version parsing, and command-line handling. Pydantic, OpenAI, Requests, HTTPX, and other libraries have no inherited status.

### 5. Select the smallest reversible representation and interface

Prefer cohesive modules directly under `src/upgradepilot/` until implemented responsibility demonstrates a subpackage boundary.
...
Select one bounded user-facing interface, likely a minimal command or Python application function accepting a repository and PR number, only after comparing the simplest credible options.
...
Conditional responsibilities activate only when the case requires them. Upstream evidence, deeper CI responsibility inspection, stopping evaluation, targeted-check design, failure attribution, dynamic reproduction, and similar work must not become mandatory merely because they appeared in simulations.

### 7. Define B2 acceptance tests

At minimum:

- a real public smoke path accepts a repository and Dependabot PR locator, freezes the exact proposal, and reaches a bounded result or honest abstention;
- deterministic tests use captured responses or normalized evidence without live-network dependence;
- invalid repository or PR input is rejected;
- a changed head is detected rather than silently mixed with earlier evidence;
- an unsupported dependency-change shape produces an explicit unsupported state or abstention;
...
Additional same-action, action-change, and broader dependency-shape cases belong when the implemented responsibility reaches them.
...
- implement or materially modify one central acquisition, identity, extraction, evaluation, or output behavior;
- add or change one meaningful test;
- diagnose one deliberately introduced acquisition, identity, evidence-authority, or output defect;
...
4. smallest current dependency, representation, and interface decision;
5. B2 acceptance and Ali ownership gates;
6. one bounded B2 implementation plan;
7. no additional ADR because the current reversible choices did not require one.

Do not create competing implementation plans, architecture variants, or a separate replay product path.
...
- security, untrusted-input, credentials, permissions, rate limits, and target-mutation boundaries are explicit at the required entry depth;
- B2 tests and ownership work are concrete;
- Ali can explain why this is the smallest credible real end-to-end product slice;
- the bounded B2 plan is active.

## Handoff

B1 no longer controls ordinary implementation. Continue through:

- [`B2_PUBLIC_PR_VERTICAL_SLICE_PLAN.md`](B2_PUBLIC_PR_VERTICAL_SLICE_PLAN.md) for the active checklist and proof boundary;
- [`../OPERATING_GUIDE.md`](../OPERATING_GUIDE.md) for learning and execution;
- [`../working-memory/B2_TECHNICAL_PROGRESS.md`](../working-memory/B2_TECHNICAL_PROGRESS.md) for detailed technical evidence;
