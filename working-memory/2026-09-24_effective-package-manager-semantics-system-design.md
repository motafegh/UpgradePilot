...
- **Current stop point — step-local `env: PATH` interaction remains open:** runner source shows the step environment is evaluated/merged and the accumulated prepend-path state is also applied before script execution, but the exact claim-level precedence/composition rule for `steps[*].env.PATH` versus accumulated `GITHUB_PATH`/`setup-python` additions has not yet been closed for UpgradePilot's supported boundary. Do not record a final precedence rule yet. This is the next micro-investigation before moving to bounded virtual-environment activation.
