#!/usr/bin/env bash
# Execute the corrected one-response Gemma E4B v1.2 completion recovery.
...
EVIDENCE="$REPO_ROOT/working-memory/evidence/2026-07-29-gemma-e4b-v1.2-completion-recovery-load-flag-correction"
DIAGNOSTIC="$EVIDENCE/diagnostic.py"
RESULT="$REPO_ROOT/working-memory/2026-07-29_B2-gemma-e4b-v1.2-completion-recovery-load-flag-correction-result.md"
...
if [[ -e "$EVIDENCE/run-summary.txt" || -d "$EVIDENCE/runs" || -e "$RESULT" ]]; then
  echo "Existing corrected recovery evidence was found and will not be overwritten." >&2
...
echo
echo "Corrected completion-recovery execution and evidence capture finished."
