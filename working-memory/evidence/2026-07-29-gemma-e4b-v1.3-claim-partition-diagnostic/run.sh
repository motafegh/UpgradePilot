#!/usr/bin/env bash
# Execute the bounded Gemma E4B v1.3 claim-partition diagnostic.
...
EVIDENCE="$REPO_ROOT/working-memory/evidence/2026-07-29-gemma-e4b-v1.3-claim-partition-diagnostic"
DIAGNOSTIC="$EVIDENCE/diagnostic.py"
RESULT="$REPO_ROOT/working-memory/2026-07-29_B2-gemma-e4b-v1.3-claim-partition-diagnostic-result.md"
...
if [[ -e "$EVIDENCE/run-summary.txt" || -d "$EVIDENCE/runs" || -e "$RESULT" ]]; then
  echo "Existing v1.3 claim-partition evidence was found and will not be overwritten." >&2
...
if [[ "$workflow_rc" -eq 0 ]]; then
  "$PYTHON" "$DIAGNOSTIC" preflight \
    > "$EVIDENCE/preflight.stdout.txt" \
    2> "$EVIDENCE/preflight.stderr.txt" || workflow_rc=22
fi

if [[ "$workflow_rc" -eq 0 ]]; then
  "$PYTHON" "$DIAGNOSTIC" snapshot pre-load \
    > "$EVIDENCE/pre-load-snapshot.stdout.txt" \
    2> "$EVIDENCE/pre-load-snapshot.stderr.txt" || workflow_rc=23
fi
...
if [[ "$workflow_rc" -eq 0 ]]; then
  if ! "$PYTHON" "$DIAGNOSTIC" snapshot post-load \
    > "$EVIDENCE/post-load-snapshot.stdout.txt" \
    2> "$EVIDENCE/post-load-snapshot.stderr.txt"; then
    workflow_rc=25
  fi
fi
...
if [[ "$workflow_rc" -eq 0 ]]; then
  ...
  "$PYTHON" "$DIAGNOSTIC" run-all
  diagnostic_rc=$?
  printf '%s\n' "$diagnostic_rc" > "$EVIDENCE/diagnostic.exit-code.txt"
  if [[ "$diagnostic_rc" -eq 2 ]]; then
    semantic_stop=1
  ...
  stop_logs
  "$PYTHON" "$DIAGNOSTIC" snapshot post-diagnostic \
    > "$EVIDENCE/post-diagnostic-snapshot.stdout.txt" \
    2> "$EVIDENCE/post-diagnostic-snapshot.stderr.txt" || true
fi
...
echo
echo "v1.3 claim-partition execution and evidence capture finished."
...
if [[ "$semantic_stop" -eq 1 ]]; then
  ...
  echo "Do not rerun or resume Gate B before independent review." >&2
fi
