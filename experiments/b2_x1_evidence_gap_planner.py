"""Evidence-refined model boundary for the B2/X1 ``EvidenceGapPlanner`` experiment.

This module is experiment support code, not UpgradePilot product runtime code.

R2 and R3 deliberately separated the full trusted orchestration state from the smaller
observation an LLM needs for evidence-gap planning.  This module implements that boundary
without yet calling a model or executing an investigation::
    ...
    -> future deterministic admission/execution

The model sees semantic planning information.  Exact repository/revision/path identity,
action preconditions, mutation policy, result-class contracts, provider retry policy, and
other execution authority stay outside this module's request payload.

Historical B2/X1 experiment modules remain unchanged as evidence.  This file is the R4-A
reference/control implementation of the evidence-refined contracts rather than a compatibility
rewrite of the earlier prototype.
"""
...
class EvidenceGapPlannerContext:
    ...
    information R2 justified as useful for model reasoning.  Deterministic execution authority
...
def render_evidence_gap_planner_request(
    ...
    """Render the exact evidence-refined payload admitted to the future model call.

    Fields are enumerated explicitly rather than produced with ``dataclasses.asdict``.  This is
    an authority boundary: adding a field to a trusted internal type must never automatically
    expose that field to the model.
    """
