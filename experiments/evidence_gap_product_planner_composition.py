"""Compose real UpgradePilot product outputs into the bounded EvidenceGapPlanner context.

This module is experiment support code, not UpgradePilot product runtime code.

The current planner comparison path must not hand-reconstruct S001 planner facts. This seam
accepts the normal ``PublicPullRequestInvestigation`` result for the existing pre-target
Python-support-drop responsibility and only projects semantics already owned elsewhere::
...
def compose_pre_target_python_support_planner_context(
    ...
    *,
    ...
    """Project one real pre-target Python-support investigation into planner context.
...
def _bound_action_for_selection(
    ...
    """Rebind the product selector output to the existing exact action definition."""
