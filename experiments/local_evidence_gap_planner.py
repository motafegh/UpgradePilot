"""Bounded local-model request/response seam for the ``EvidenceGapPlanner``.

This module is experiment support code, not UpgradePilot product runtime code.

The planner model-boundary module owns the model-visible context projection and strict
``EvidenceGapDecision`` parser. Deterministic action admission owns fresh action rebinding and
execution authority. This module adds only the local-model transport boundary between them::
...
remains untrusted semantic output and still requires deterministic admission before any selected
investigation may execute.
"""
