"""Fresh deterministic action admission for the B2/X1 ``EvidenceGapPlanner`` experiment.

This module is experiment support code, not UpgradePilot product runtime code.

R4-A1 defines the smaller model-visible observation and the untrusted ``EvidenceGapDecision``.
R4-A2 owns the next authority boundary: a model-selected action ID is only a proposal.  Before
execution, deterministic code must rebind that ID to the current trusted exact action definition
and re-check the latest trusted state::
