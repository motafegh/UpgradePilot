"""Apply one admitted EvidenceGapPlanner investigation transition and preserve replayable proof.

This module starts after deterministic action admission. It does not decide which action should
...
class EvidenceGapInvestigationState:
    """Small evolving trusted state for the bounded ordinary-Python planner loop."""
...
class EvidenceGapTransitionTrace:
    """Deterministic record of one bounded investigation transition.
...
def run_evidence_gap_transition(
    ...
    *,
    ...
    ``ACTION_SELECTED`` must already have passed deterministic admission. No-action decisions
    bypass capability execution entirely. The function intentionally does not run another planner
    turn.
    """
    ...
    if admitted_action.action.action_id != TARGET_PYTHON_DECLARATION_ACTION_ID:
        raise ValueError("the current transition seam supports only the target-Python action.")
    if admitted_action.action.action_id in state.consumed_actions:
        raise ValueError("an already-consumed action cannot enter investigation execution.")
    if state.remaining_investigations <= 0:
        raise ValueError("investigation execution requires remaining investigation budget.")
...
def _require_active_state(state: EvidenceGapInvestigationState) -> None:
    if state.continuation_status != "ACTIVE":
        raise ValueError("only ACTIVE investigation state may enter another transition.")
