"""Framework-neutral semantic projection for bounded evidence-gap implementation comparison.

This module is experiment/evaluation support. It does not define product runtime state and it does
not require the ordinary-Python and LangGraph implementations to share internal classes, topology,
or trace representation. Instead it projects each implementation's observable result into the
small semantic surface that the comparison actually cares about.

The projection is intentionally suitable for later implementation comparisons as well: comparison
should normalize accepted behavior, not force one implementation's architecture onto another.
"""
...
from experiments.evidence_gap_action_admission import EvidenceGapAdmissionProblem
...
class EvidenceGapSemanticProjection:
    """Small common semantic surface used to compare evidence-gap implementations."""
...
def project_ordinary_python_transition(
    trace: EvidenceGapTransitionTrace,
) -> EvidenceGapSemanticProjection:
    """Project one completed ordinary-Python transition into the common comparison surface."""
...
def project_ordinary_python_admission_rejection(
    state: EvidenceGapInvestigationState,
    decision: EvidenceGapDecision,
    problem: EvidenceGapAdmissionProblem,
) -> EvidenceGapSemanticProjection:
    """Project an ordinary-Python action proposal rejected at fresh admission."""

    if decision.decision_kind != "ACTION_SELECTED":
        raise ValueError(
            "ordinary-Python admission rejection comparison requires ACTION_SELECTED."
        )
    if decision.action_id != problem.action_id:
        raise ValueError(
            "ordinary-Python decision and admission problem action ids must match."
        )
...
def project_langgraph_result(
    result: EvidenceGapLangGraphResult,
) -> EvidenceGapSemanticProjection:
    """Project one completed LangGraph result into the common comparison surface."""
    ...
    if isinstance(planner, EvidenceGapLangGraphActionProposal):
        ...
        raise TypeError("unsupported LangGraph planner outcome for comparison.")
    ...
    if authority is None:
        ...
        raise TypeError("unsupported LangGraph authority outcome for comparison.")
