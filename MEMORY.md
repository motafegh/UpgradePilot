...
- **Current responsibility:** **AUDIT-008-F4 — CI consuming-job identity → Target artifact-environment composition; Build B PARTIAL**. A preflight is complete; Target's selected-job API and focused test source are committed, but **normal application integration, application-test migration and executable F4 validation are pending**. Do not mark F4, its D/E learning or any non-abstention maintainer action complete.
- **Primary operation:** bounded Build/Implement + canonical A → B → C → D → E Learning-by-Doing. Source/test implementation has been explicitly authorized by Ali after the preceding action-relative comparison. Learning-note artifacts remain out of scope unless explicitly requested.
...
**Product issue:** `ci/consumption.py::StaticDependencyConsumptionEvidence` already carries an exact `job_key` for a supported static dependency-consuming CI job. `src/upgradepilot/investigation.py::_compose_target_artifact_environments` currently discards that key from its deduplication relation and does not pass it to `target/artifact_environment.py`; the old Target one-job selection safely reports ambiguity for a two-job workflow even when CI identified the consuming job. The closed action-relative comparison selected F4 as one bounded composition repair, not an immediate action-unlocking feature.

**Committed F4 Target-side increment:** `src/upgradepilot/target/artifact_environment.py` accepts an optional explicit `consuming_job_key`, interprets only the exact matching static workflow job, rejects absent/unsupported/ambiguous identity and preserves the original one-job-only direct-call behavior when no job key is supplied. `tests/test_target_selected_consuming_job.py` contains source for focused two-job positive, missing-key, reusable-job, empty-key and legacy-direct-call cases. Source commit `a6f0ede6d89eed8d60c979f23a282e43f896819c`, test commit `0d3f32396288a4572b4b2632d0ddc1e2e9af78ee`. These are **committed source, not passing test evidence**.

**Next bounded implementation work (still F4, not a new feature):** connect the existing supported consumption job key through normal application composition; include the job in deduplication, verify exact workflow/revision/source/repository relationship without inventing Target domain facts, preserve job-specific results, and update the historical application test that currently expects two-job ambiguity to assert the selected consuming job instead. Add only distinct wrong/missing job and multiple-consuming-job integration tests. Review the actual source/changed-file diff, execute focused Target + investigation tests and the broader deterministic regression on the changed revision, and record any hosted proof separately. A historical F9 green run does not prove F4.

**Learning:** Ali now understands that `test`/`lint` were example *job names*, not missing CI test commands; CI already inspects them. During F4 D, explain actual source key propagation and why the resulting environment declaration is not runtime dependency installation, exact wheel compatibility, behavioral safety or a permission to merge/block. Do not claim independent source-level ownership before that discussion.
