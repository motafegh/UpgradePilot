...
## First bounded M2-session constraints

The session must:

1. use the completed `pydantic/pydantic#13432` M1 report as the real case;
2. define one small machine responsibility rather than an entire architecture;
...
The prior JSON example, CLI, policy, package structure, and tests are not an authorized starting point.
...
Define and activate the first bounded M2 learning/implementation session using the completed Pydantic case and the constraints above. Do not restore the previous scaffold or assume any retained architecture proposal is accepted.
