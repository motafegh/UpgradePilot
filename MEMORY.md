...
Exact historical filenames or identifiers may remain only where needed for provenance; they are not current vocabulary. Widely established technical terms such as HTTP, JSON, API, WSL, TOML, LangGraph, and LangChain remain acceptable when they improve clarity.
...
## Current framework evidence horizon
...
The ordinary-Python implementation is evidence/reference, not architecture authority for later framework designs.
...
## Current core-product pressure

The normal `PublicPullRequestInvestigation` application path already coordinates substantial deterministic evidence work, including dependency identity, CI evidence, package/release evidence, upstream repository/release/changelog evidence, Python-support interpretation, target relevance, and candidate applicability.

A materially different second mechanism also exists partially in product source:

```text
artifact-serviceability / installation-mode candidate and applicability logic
+
